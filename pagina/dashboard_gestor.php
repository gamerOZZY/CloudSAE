...
function subirImagenAzure($archivoTmp, $nombreArchivo){
    ...
    // DESACTIVAR temporalmente la verificación SSL
    ...
    if($httpCode == 201) {
        ...
        die("<div style='background:#fff; color:#000; padding:20px; z-index:9999; position:relative;'>
                <h3>Error fatal al subir a Azure</h3>
                <p><strong>Código HTTP:</strong> $httpCode</p>
                <p><strong>Error cURL:</strong> " . ($curlError ? $curlError : 'Ninguno') . "</p>
                <p><strong>Respuesta de Azure:</strong> " . htmlspecialchars($respuesta) . "</p>
             </div>");
    }
}
...
if(isset($_POST['alta_alumno'])){
    try {
        ...
        $fotoURL = !empty($_FILES['foto']['tmp_name']) ? subirImagenAzure($_FILES['foto']['tmp_name'], $_FILES['foto']['name']) : "";

        // 1. Insertar Persona
        $stmt = $pdo->prepare("INSERT INTO Persona (nombre_completo, contrasena, foto_perfil, id_escuela) VALUES (?,?,?,?)");
        $stmt->execute([$_POST['nombre'], $_POST['contrasena'], $fotoURL, $_POST['escuela']]);
        ...
        // 3. Insertar Materia (Tiene_Inscrita)
        ...
        $pdo->commit();
        $mensaje = "Alumno registrado exitosamente con carga académica.";
    } catch(Throwable $e) {
        $pdo->rollBack();
...
if(isset($_POST['baja_alumno'])){
    ...
    $mensaje = "Alumno eliminado del sistema.";
}
...
/* --- ACTUALIZAR ALUMNO (ESTRICTO: SIN CALIFICACIONES) --- */
if(isset($_POST['actualizar_alumno'])){
    $idAlumno = $_POST['id_alumno'];
    
    // Actualizar Persona
    $stmt = $pdo->prepare("UPDATE Persona SET nombre_completo=?, id_escuela=? WHERE id_persona=?");
    $stmt->execute([$_POST['nombre'], $_POST['escuela'], $idAlumno]);
    
    // Actualizar Alumno
    $stmt = $pdo->prepare("UPDATE Alumno SET boleta=?, edad=? WHERE id_persona=?");
    $stmt->execute([$_POST['boleta'], $_POST['edad'], $idAlumno]);

    // REGLA DE NEGOCIO CUMPLIDA: Bloqueo explícito de campos de calificación.
    // Solo se permite alterar el semestre o la materia.
    ...
    $mensaje = "Datos del alumno actualizados (Calificaciones protegidas).";
}
...
if(isset($_POST['actualizar_profesor'])){
    ...
    $stmt = $pdo->prepare("UPDATE Persona SET nombre_completo=?, id_escuela=? WHERE id_persona=?");
    $stmt->execute([$_POST['nombre'], $_POST['escuela'], $idProfesor]);
...
// ==========================================
// 3. CONSULTAS PARA VISTAS Y CATÁLOGOS
// ==========================================
$escuelas = $pdo->query("SELECT id_escuela, nombre FROM Escuela")->fetchAll(PDO::FETCH_ASSOC);
$materias = $pdo->query("SELECT id_materia, nombre FROM Materia")->fetchAll(PDO::FETCH_ASSOC);

$optsEscuela = ""; foreach($escuelas as $e) $optsEscuela .= "<option value='{$e['id_escuela']}'>{$e['nombre']}</option>";
$optsMateria = ""; foreach($materias as $m) $optsMateria .= "<option value='{$m['id_materia']}'>{$m['nombre']}</option>";

/* ALUMNOS */
$stmt = $pdo->query("
    SELECT a.id_persona, a.boleta, a.edad, p.nombre_completo, p.foto_perfil, p.id_escuela, 
...
/* PROFESORES */
$stmt = $pdo->query("
    SELECT pr.id_persona, pr.numero_empleado, pr.tipo, p.nombre_completo, p.foto_perfil, p.id_escuela
    FROM Profesor pr
...
<div class='form-container'>
    ...
    <form method='POST' enctype='multipart/form-data'>
        <div class='form-grid'>
            ...
            <div class='form-group'><label>Escuela</label><select name='escuela' required>$optsEscuela</select></div>
...
// Tabla interactiva para Modificaciones (UPDATE)
$htmlModificaciones = "<h3 style='color:var(--wine-pale); margin-bottom:15px;'>Modificar Alumnos (Cero Calificaciones)</h3><div class='table-container' style='margin-bottom:30px;'><table class='data-table'><thead><tr><th>Boleta</th><th>Nombre</th><th>Edad</th><th>Semestre</th><th>Acción</th></tr></thead><tbody>";
foreach($listaAlumnos as $a){
    $optsEsc = str_replace("value='{$a['id_escuela']}'", "value='{$a['id_escuela']}' selected", $optsEscuela);
    $htmlModificaciones .= "<tr><form method='POST'>
        <input type='hidden' name='id_alumno' value='{$a['id_persona']}'>
        <input type='hidden' name='id_materia_original' value='{$a['id_materia']}'>
        <input type='hidden' name='escuela' value='{$a['id_escuela']}'>
        <td><input type='text' name='boleta' class='input-grade' value='{$a['boleta']}' style='width:90px;'></td>
        <td><input type='text' name='nombre' class='input-grade' value='{$a['nombre_completo']}' style='width:180px; text-align:left;'></td>
        <td><input type='number' name='edad' class='input-grade' value='{$a['edad']}' style='width:60px;'></td>
        <td><input type='number' name='grado_semestre' class='input-grade' value='{$a['grado_semestre']}' style='width:60px;'></td>
        <td><button type='submit' name='actualizar_alumno' class='btn-save'>Actualizar</button></td>
    </form></tr>";
}
$htmlModificaciones .= "</tbody></table></div>";

$htmlModificaciones .= "<h3 style='color:var(--wine-pale); margin-bottom:15px;'>Modificar Profesores</h3><div class='table-container'><table class='data-table'><thead><tr><th>No. Emp</th><th>Nombre</th><th>Tipo</th><th>Acción</th></tr></thead><tbody>";
foreach($listaProfesores as $p){
    $optsEsc = str_replace("value='{$p['id_escuela']}'", "value='{$p['id_escuela']}' selected", $optsEscuela);
    $htmlModificaciones .= "<tr><form method='POST'>
        <input type='hidden' name='id_profesor' value='{$p['id_persona']}'>
        <td><input type='text' name='numero_empleado' class='input-grade' value='{$p['numero_empleado']}' style='width:90px;'></td>
        <td><input type='text' name='nombre' class='input-grade' value='{$p['nombre_completo']}' style='width:180px; text-align:left;'></td>
        <td><input type='text' name='tipo' class='input-grade' value='{$p['tipo']}' style='width:90px;'></td>
        <td style='display:none;'><select name='escuela'>$optsEsc</select></td>
        <td><button type='submit' name='actualizar_profesor' class='btn-save'>Actualizar</button></td>
    </form></tr>";
}
$htmlModificaciones .= "</tbody></table></div>";
...
<head>
  ...
  <style>
    /* =============================================
       CSS VARIABLES — PALETA (SIN MODIFICAR)
       ============================================= */
    :root {
      --wine-dark:   #62152d;   
      --wine-mid:    #952f57;   
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* --- ESTILOS AGREGADOS PARA FORMULARIOS (Mimetizados) --- */
    ...
    @media (max-width: 1024px) { .sidebar { width: 220px; min-width: 220px; } .panel-text { padding: 30px; } }
    @media (max-width: 768px) { body { overflow: auto; } .app { flex-direction: column; height: auto; min-height: 100vh; } .sidebar { width: 100%; min-width: 0; border-right: none; border-bottom: 1px solid var(--border-faint); overflow: visible; } .nav-wrap { flex-direction: row; overflow-x: auto; padding: 0 12px 6px; scrollbar-width: none; -webkit-overflow-scrolling: touch; gap: 8px; } .nav-wrap::-webkit-scrollbar { display: none; } .nav-btn { flex-shrink: 0; min-width: 148px; width: auto; } .content-area { padding: 12px; flex: 1; min-height: 70vh; } .panel-text { padding: 26px 22px; overflow: visible; } .sidebar-cta { display: none; } }
  </style>
</head>
...
<body>
  ...
  <div class="ticker" role="marquee">
    <div class="ticker-track">
      <span class="ticker-item"><span class="ticker-sep"></span>Auditoría de Gestor Activa</span>
      <span class="ticker-item"><span class="ticker-sep"></span>Alta de ciclo operativo</span>
      <span class="ticker-item"><span class="ticker-sep"></span>Sincronización con Azure Blob Storage correcta</span>
    </div>
  </div>
    ...
    <aside class="sidebar">
      <div class="sidebar-logo">
        <div class="logo-row">
          ...
          <div class="logo-text-wrap">
            ...
            <div class="logo-tagline">Panel Gestor</div>
          </div>
        </div>
      </div>
      ...
      <nav class="sidebar-nav" aria-label="Secciones">
        <div class="nav-wrap" id="navWrap">
          ...
          <button class="nav-btn" data-section="2" data-color="3"><div class="nav-btn-inner"><span class="nav-num">03</span><span class="nav-label">Modificaciones</span><span class="nav-arrow">↗</span></div></button>
        </div>
      </nav>
  ...
  <script>
    const SECTIONS = [
      {
          tag: 'Alumnos', title: 'Gestión de <em>alumnos.</em>', sqlQuery: 'INSERT / DELETE Alumno',
          ...
          data: <?= json_encode(array_map(fn($a)=>[
              ...
              $a["boleta"], $a["nombre_completo"], $a["grado_semestre"], $a["materia_nombre"],
      ...
      {
          tag: 'Profesores', title: 'Gestión de <em>profesores.</em>', sqlQuery: 'INSERT / DELETE Profesor',
          ...
          data: <?= json_encode(array_map(fn($p)=>[
              ...
              $p["numero_empleado"], $p["nombre_completo"], $p["tipo"],
      ...
      {
          tag: 'Modificaciones', title: 'Actualización de <em>datos.</em>', sqlQuery: 'UPDATE SET (Restricción de Calificaciones Activa)',
    ...
    function renderSection(idx) {
      ...
      if (s.type === 'form_table') {
        ...
        const tbody = s.data.map(row => `<tr>${row.map(cell => `<td>${cell}</td>`).join('')}</tr>`).join('');
        contentHtml = `${s.formHtml} ${sqlBadge} <div class="table-container"><table class="data-table"><thead><tr>${thead}</tr></thead><tbody>${tbody}</tbody></table></div>`;
    ...
    let currentIdx = 0; let isAnimating = false;
